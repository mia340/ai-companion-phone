...
穿搭页不再是一条需要来回上下翻的长表单。上半屏固定保留角色预览，下半屏使用人物 / 头发 / 上装 / 下装 / 配饰 / 套装六个分类编辑器；只有编辑区内部滚动。

公共小人改为正面 64×82 SVG 像素骨架，重点保证脸部、发型、衣服和男女版型在地图与衣橱中都可辨认。视觉参考温暖 16-bit 农场 RPG 的清晰像素层次，但不使用或复制任何现成游戏素材。
