...
- WorldBook / Regex 从“角色所有权”改为共享资源库：角色卡来源只记录 provenance，实际启用关系全部由 `ResourceBinding` 决定；同一资源可同时绑定多个角色或全局启用。
- 旧角色专属 WorldBook / Regex 自动迁移为“共享资源 + 原角色默认绑定”；旧散装 Lorebook 条目自动收拢成共享书，避免清空角色 ID 后误变成全局常驻。
- 删除角色或给角色换卡时不再自动删除其来源 WorldBook / Regex；只解除角色绑定，资源继续留在共享资源库，必须由用户明确从资源库删除。
- 世界书条目新增“查看全文”阅读器：长 HTML / CSS / 原始协议文本可纵向滚动、自动换行并复制全文，不再被卡片宽度截断。
- 角色详情以一份“完整角色介绍”为主；旧卡若只有 personality 被写进 persona，V14 会在有原始 description/personality 时安全回填完整介绍。说话方式、背景、喜欢/不喜欢等只有真的有值才显示，不再用“未填写”占位。
- 角色创建/编辑不再强迫把社区卡拆成性格、说话方式和背景；无法可靠拆分时保留原文作为完整角色介绍。
- Backup V9 恢复时同样执行 V14 共享资源/去分组归一，避免旧备份把旧结构重新写回当前数据库。
- Prompt Preset 宏优先读取原卡 `cardDescription` / `cardPersonality`，不再拿不可靠的拆分字段代替原卡语义。
- Backup 版本仍为 V9。
