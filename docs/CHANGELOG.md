...
- 互动面板改为可上下拖动的三段高度，默认中档；新增“看地图 / 展开·收起”，并允许透过遮罩操作顶部返回。
- 游戏模式固定为现实面对面；房间、酒店等实际地点不预设，只有用户明确说出时才可进入上下文。
- 棋盘地点明确降级为虚构 Board Stop：花店、公园、电影院等不得被模型当成现实场景或长期事实。
- 每道题严格归本回合掷骰者；对方只做自然回应，不再出现要求双方各自回答的系统题。
- 对话气泡去掉重复姓名标签，减少视觉噪声。
- 旧的聊天专用大冒险已改写为面对面版本；Memory 回忆出题也加入“单题单归属”约束。
...
- PWA 启动主动检查更新并清理过期缓存，降低刷新后继续命中旧 UI 的情况。
- 搭档选择支持触摸横滑、鼠标拖动与滚轮，不再只够得到前三张卡。
- 内置题库扩为 128 道，并默认打开系统题视图。
- 骰子动画改为有限次，动画结束与 IndexedDB 保存解耦；异常路径也会解除 rolling 状态。
- 数据版本不变；测试定义 67 files / 598 declarations。

## V0.5.0-alpha.5.7.0 · 心跳飞行棋 V1.2

- 新增独立「情侣内容中心」：自定义真心话/大冒险支持新增、编辑、删除、搜索、题型/强度筛选；系统题可逐条启停，题库可 JSON 导入导出。
- 情侣事件卡开放自定义：支持双方心动变化、交换位置、当前玩家再掷一次；事件牌组和系统事件启停状态会在开局冻结，局中编辑不污染存档。
- 新增本地「情侣回忆册」：完成棋局自动归档最近 24 局，保存确定性高光、心动/挑战/事件/证据题统计；不自动写入 Memory。
- AI 出题 V2 同时读取经过筛选的真实 Memory 与 Shared Timeline evidence；承诺/目标/故事信号继续排除，timeline evidence 使用独立 id 并由 Runtime 强校验。
- 新增结算主动带到聊天：只写本地草稿，用户自行发送；游戏结果不自动成为关系事实。
- 题库/事件全禁用时对应棋盘格安全作为 pass-through，不会因空牌组崩溃；旧 V1/V1.1 snapshot 与偏好继续兼容。
- Router 进一步拆分编辑器、设置页和次级页面为 route chunks，降低首屏主包压力；IndexedDB V18、Backup V12、Launcher Grid V13 / HomeLayout revision 13 不变。
- 静态测试定义提升为 67 files / 597 declarations，其中 Couple Board 直接相关 46 条。
