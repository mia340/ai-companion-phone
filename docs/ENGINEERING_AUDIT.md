...
长期幽灵页问题暴露出“自动修复逻辑多，但缺少可观测证据”的风险。alpha.5.1.22 新增数据/DOM 双层 HomeLayout Inspector；后续所有 Launcher 自愈都应先产出可解释诊断，再决定是否迁移/回收。相同原则也应用到 Prompt Runtime：WorldBook 与 Context Inspector 记录触发依据、预算与来源，避免调参只能靠肉眼比较最终回复。
...
- 当前测试定义：42 个测试文件、349 个 `it/test` 声明；除 Conversation Runtime 外，朋友圈、音乐、海龟汤和 Provider 也已有纯规则/请求体回归测试。
