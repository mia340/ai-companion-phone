...
Launcher 的稳定状态继续以 `homeLayoutPages[]` 为事实源，但 revision 12 增加一条结构不变量：**同一 App 不得同时存在于 Dock 与桌面/文件夹**。`normalizeHomeAppearance()` 在进入 Grid 归一化前先确定 Dock，再从桌面允许集合中排除 Dock App；这与真实手机“把 App 拖进 Dock 即离开原槽位”的语义一致，也能清理历史升级过程中产生的重复幽灵项。

HomeLayout Inspector 分成两层：

```text
Persisted Layout Inspection
  ├─ page/item/type/key/x/y/w/h
  ├─ bounds / overlap / duplicate / Dock duplicate
  └─ occupied cells
         +
Runtime DOM Inspection (current page)
  ├─ DOM present
  ├─ proof target (.app-icon / folder tile / widget)
  ├─ ancestor visibility + cumulative opacity
  └─ elementFromPoint painted check
```

因此“页数组有项目但用户看到空白”可以区分成数据异常、DOM 缺失和 compositor/paint 异常，而不再统一归类为 `items=[]`。诊断 JSON 可直接复制，修复器只处理可以确定的结构问题；视觉空白页强制回收前仍先写 recovery snapshot。

Generation 调试继续向可解释编译器演进。WorldBook 决策不仅记录是否激活，还保留 primary/secondary keyword 命中、selective logic、scan depth、regex/case/whole-word 语义、递归层与预算信息；Prompt section 同时记录字符数和估算 token，Context Inspector 可以把 Character / Persona / Preset / Memory / WorldBook / Recent Messages 与 Provider 实际 prompt token 对照。
...
HomeAppearancePreferences
  ├─ homeAppKeys[]       # 兼容 / 成员集合
  ├─ homePageKeys[][]    # 非空页归属与顺序
  ├─ dockAppKeys[]       # 固定 Dock，最多 4
  └─ homeWidgetKeys[]

Pointer Gesture
  ↓
currentPage + swipeOffset
  ↓
transform: translate3d(...)
```

主屏不再使用浏览器原生 `overflow-x:auto` / scroll snap 作为分页状态源，而由 Pointer Events 驱动 `currentPage` 与临时 `swipeOffset`，页轨道使用 transform 呈现。这样触屏与鼠标都可以直接左右拖动翻页，同时避免浏览器原生 scrollbar / edge indicator 泄漏进 OS-like 桌面。

页面不是可人工创建的独立实体。`normalizeHomeAppearance()` 会清理空页；整个桌面至少一页。第一页有 Widget 时容量为 4 个 App，其余页容量为 12；内容溢出才自动新增页，某页最后一个 App 被移走后该页自动回收。跨页拖拽产生的非空页归属仍然持久化。

`PhoneFrame lockScroll` 继续关闭外层纵向 scroll layer；Dock 独立于分页，最多 4 个 App。旧版本 `homePageKeys` 中遗留的人为空页会在加载/保存时无迁移自动收敛。
