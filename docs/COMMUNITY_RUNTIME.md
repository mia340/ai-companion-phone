...
## V0.5.0-alpha.5.1.22 WorldBook Activation Inspector / Context Inspector

WorldBook Engine V2 的调试输出不再只保存“激活了哪些条目”。每个条目可以记录：

- activation kind / status；
- primary / secondary keyword 命中；
- selective logic；
- scan depth；
- regex / case-sensitive / whole-word 模式；
- match score / recursion depth；
- estimated token / priority / insertion order / position / probability。

Prompt Debug 同时把 `promptSections` 的实际片段转为字符数 + 估算 token，并单独显示 Character Card、Persona、Prompt Preset、Memory、WorldBook、Recent Messages、Regex 与 Agent/Interaction 来源。该 Inspector 只解释实际冻结后的 Generation Context，不用设置页的“启用状态”代替真实输入。
