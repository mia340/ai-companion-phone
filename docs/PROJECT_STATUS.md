...
项目已经形成单角色沉浸式陪伴聊天闭环，完成多模态图片理解、流式回复、语音输入、角色朗读和消息可靠性升级，并开始拆分超大型聊天页面组件。
...
- 拆分 `ChatHeader.vue`、`ChatMessageList.vue`、`ChatActionSheet.vue`
- 拆分 `ChatImagePreview.vue` 与 `ChatSettingsPanel.vue`
- 增加更直观的角色音色预设与试听
- 一次发送 2～4 张图片，并支持相机直接拍摄
- 视觉内容确认后写入长期记忆
