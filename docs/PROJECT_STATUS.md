...
项目已经进入“角色内核 + 小手机动作协议”阶段：除角色卡 V2、Persona、世界书和 Prompt 编排外，角色还能以连续短消息、表情、语音样式和结构化状态进行互动，并可通过 Prompt 调试器检查世界书、记忆和模型原始输出。
...
备份格式为 V6，可选择是否包含图片，并兼容导入 V1～V5。
...
- 高级世界书：次关键词、AND/NOT、概率、扫描深度和插入位置
- 模型专属 Roleplay Preset 与 Prompt Manager
- Prompt 模块排序、开关和上下文预算
- 世界书触发原因与脱敏调试报告
- 继续提取 `useChatStreaming.ts`、`useMessageActions.ts` 和互动协议 composable
