...
- “当前相处状态”与“聊天呈现方式”正式解耦：presence 只记录世界事实；用户可独立选择场景合并、纯手机消息、动作/台词分开三种呈现，默认场景合并。
- 自动相处状态补强 Persona / 在场角色 / 自然视线语义：`😶在场角色` 中出现当前 Persona、`垂眸看着你` 等明确同场证据可直接判为 together，并继续继承上一轮已确定状态。
- 新增资源会话持续：微信/论坛等作者按需资源一旦显式打开，后续短句默认继续作用于当前资源；明确退出或切换其它资源后结束。大型资源续聊只发送作者入口规则 + 会话延续指令，不再每轮重复注入整份 HTML 说明书。
- Regex 增加确定性结构容错：作者写 `[字段]`、模型输出 `【字段】` 时仅在 Regex 输入视图做本地括号兼容，不修改原作者 Regex，不追加 AI 调用。
- 社区 HTML 合同增加本地格式修复：当 AI 已给出状态事实与正文、只是漏掉作者 HTML 外壳时，本地套回原模板并把缺失的扩展栏目留空，不编造 NPC/观众内容；本地无法确认只是格式问题时才允许模型纠偏。
- 当前资源 UI 会话期间即使用户启用“小手机增强”，也暂停 `companion_packet/scene_action` 私有整形，避免社区资源界面被第二套协议抢占。
- IndexedDB V14 / Backup V9 不变；新增字段均为现有对象的可选字段，无需数据库迁移。

## V0.4.4.4 当前状态

- 角色卡拆分定位为本地“原卡阅读器 / 索引”，不再等同于 Prompt 内容；Raw Card 继续是社区卡事实源。
- 社区卡默认只把作者原始角色字段送入 Prompt；本地为 UI 解析出的年龄、职业、关系、喜好等不重复注入。
- Resource Intent Router 可根据资源标题、keys 和作者触发说明识别用户本轮明确调用的功能；大型按需模块不再因为 `constant` 就无条件每轮全文发送。
- 作者明确要求每轮输出的状态栏 / 固定 UI 仍常驻，不会为了省 Token 破坏原卡合同。
- Prompt Debug 可直接查看资源 Focus / 注入 / 休眠与预计节省字符；Provider 返回 usage 时显示真实输入/输出/总 Token。后续 WorldBook Engine V2 将继续完善预生成 token budget、recursive scanning、sticky/cooldown/group scoring。
- IndexedDB V14 / Backup V9 不变。

## V0.4.4.3 当前状态

- Regex 只负责后处理；Regex 自身不再制造模型输出合同，未命中不阻止 AI 正文保存。
- 原卡明确固定格式仍由 AI 遵守；纠偏失败时回退第一版真实 AI 回复，不再出现“模型有输出但用户看到无”的静默丢失。
- Safe Rich UI 不运行第三方 JS，但能本地恢复常见静态数据填充、状态块与展开/收起交互。
- WorldBook / Regex 编辑器已开放常见社区运行字段，脚本启停、资源绑定与执行顺序彼此独立。
- 无显式 UI 的开场格式采用软连续性提示，字段值完全由 AI 根据剧情生成。
- IndexedDB V14 / Backup V9 不变。
