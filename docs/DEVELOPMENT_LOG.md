...
## 2026-09-20 · V0.5.0-alpha.5.1.22

- 对幽灵第二页做数据层与 DOM 层双诊断：不再只有一行 page summary，而是逐 Item 记录布局、DOM 存在性、当前页真实绘制命中和异常原因，并支持复制诊断 JSON。
- HomeLayout normalize 正式禁止同一 App 同时存在于 Dock 和桌面；revision 12 会自动修正历史重复项并回写。
- WorldBook Inspector 继续向“为什么触发”深化，记录 primary/secondary match、selective logic、scan depth 与匹配模式。
- Context Inspector 开始用 Prompt section 的实际文本估算 token，并增加 Preset 来源，使字符预算与 Provider 实际 token 能交叉核对。
...
## 2026-09-20 · V0.5.0-alpha.5.1.19

- 对长期残留的第二页幽灵页停止盲删，新增 HomeLayout Inspector。
- 新增安全折叠修复：视觉空白但数据非空的页，只有在项目可无损搬回前页时才删除。
- 页 DOM key 改为内容签名，降低分页重排后的 Vue DOM 复用风险。
