...
function roleCardUiRules(input: RoleplayPromptInput) {
  if (!characterRequiresRoleCardUi(input.character)) return ''
  return [
    '【角色卡自带 UI 协议】',
    '这张角色卡要求每次回复携带日期、时间、地点、内心、周围、待办状态。必须继续遵守原卡规则，不要因为小手机互动协议而丢掉它。',
    '每次回复最前面先输出以下结构，字段名保持不变：',
    '{日期:剧情日期|时间:剧情时间}',
    '{地点:当前场景}',
    '{内心:15字内，只写角色内心活动}',
    '{周围:独处/某人在场}',
    ...
    '如果 UI 的“周围”显示用户在场，或动作明显与用户发生现实接触，则双方视为同场景：动作使用 scene_action，界面会把动作以中文括号合并进剧情气泡。',
