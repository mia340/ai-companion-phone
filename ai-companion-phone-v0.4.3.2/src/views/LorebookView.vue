...
import type { Character, LorebookEntry, LorebookResource } from '../types/domain'
...
async function submit() {
  ...
  await saveLorebookEntry({
    ...
    worldId: character.value?.worldId || DEFAULT_WORLD_ID,
    lorebookId: bookId.value || undefined,
    characterId: form.characterOnly && characterId.value ? characterId.value : (book.value?.characterId || undefined),
    title: form.title,
...
onMounted(async () => {
  ...
  if (bookId.value) {
    book.value = await db.lorebooks.get(bookId.value)
    if (!character.value && book.value?.characterId) character.value = await db.characters.get(book.value.characterId)
  }
  await refresh()
})
</script>
...
<template>
  <PhoneFrame :title="book?.name || (character ? `${character.name}的世界书` : '世界书 Lorebook')" show-back>
    <main class="lore-page">
      <section class="intro-card">
        <b>{{ book ? `正在编辑：${book.name}` : '按关键词注入世界设定' }}</b>
