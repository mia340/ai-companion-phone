...
<template>
  <section class="bottom-panel" :style="panelStyle">
    ...
    <template v-if="chatSettings?.innerThoughtVisibility !== 'off'">
      ...
      <div v-if="conversationState?.location || conversationState?.presence || conversationState?.relationshipNote || conversationState?.timePeriod || conversationState?.energy" class="status-chips"><span v-if="conversationState?.presence">{{ conversationState.presence==='together'?'◉ 在你身边':'◌ 远程联系' }}</span><span v-if="conversationState?.location">📍 {{ conversationState.location }}</span><span v-if="conversationState?.timePeriod">◷ {{ conversationState.timePeriod }}</span><span v-if="conversationState?.energy">⚡ {{ conversationState.energy }}</span><span v-if="conversationState?.relationshipNote">♡ {{ conversationState.relationshipNote }}</span></div>
      ...
      <div v-if="conversationState?.unresolvedTopics?.length || conversationState?.pendingEvents?.length || conversationState?.shortTermGoals?.length" class="state-lists">
        <p v-if="conversationState?.unresolvedTopics?.length"><b>还想继续聊</b><span v-for="item in conversationState.unresolvedTopics" :key="item">{{ item }}</span></p>
        <p v-if="conversationState?.pendingEvents?.length"><b>等待中的事</b><span v-for="item in conversationState.pendingEvents" :key="item">{{ item }}</span></p>
        <p v-if="conversationState?.shortTermGoals?.length"><b>眼前目标</b><span v-for="item in conversationState.shortTermGoals" :key="item">{{ item }}</span></p>
        <p v-if="conversationState?.lastCompletedEvent"><b>刚刚完成</b><span>{{ conversationState.lastCompletedEvent }}</span></p>
      </div>
...
.state-lists{display:grid;gap:8px;margin:0 0 14px}.state-lists p{display:grid;gap:5px;margin:0;padding:11px 12px;border-radius:14px;background:#f8f1f4}.state-lists b{color:#9a6079;font-size:11px}.state-lists span{color:#765b67;font-size:12px}
</style>
