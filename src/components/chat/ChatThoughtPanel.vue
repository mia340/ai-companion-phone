...
<template>
  <section class="bottom-panel" :style="panelStyle">
    ...
    <template v-if="chatSettings?.innerThoughtVisibility !== 'off'">
      ...
      <div v-if="conversationState?.location || conversationState?.relationshipNote" class="status-chips"><span v-if="conversationState?.location">📍 {{ conversationState.location }}</span><span v-if="conversationState?.relationshipNote">♡ {{ conversationState.relationshipNote }}</span></div>
