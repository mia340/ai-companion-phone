      ...
      <div class="assistant-message-stack">
        ...
        >
          ...
          >
            <img
              ...
              class="message-image"
              @click.stop="emit('openImage', message.imageDataUrl || '')"
            />
            <span
          ...
          <template v-else>
            <span
              v-if="message.type === 'music'"
              class="music-message-mark"
            >♫</span>
            {{ message.content }}<i
              v-if="streaming"
        ...
        <small
          ...
          :class="[
            ...
            `assistant-message-state--${message.status}`
          ]"
        >
          {{ message.status === 'cancelled' ? '已停止生成' : '回复中断' }}
        </small>
      </div>
    </template>
    ...
    <template v-else>
      ...
      >
        ...
        >
          ...
          <span
            v-if="message.content"
            class="image-caption image-caption--mine"
          >
            {{ message.content }}
          </span>
        </template>
        ...
        <span
          v-else-if="message.type === 'image'"
          class="missing-image missing-image--mine"
        >
          图片未包含在这份备份中
          <small v-if="message.content">
            {{ message.content }}
...
.assistant-message-stack .bubble {
  max-width: 100%;
}
...
.assistant-message-state--failed {
  color: #c84f63;
}
...
.streaming-caret {
  ...
  vertical-align: -.16em;
  animation: stream-caret .8s steps(2, end) infinite;
}
