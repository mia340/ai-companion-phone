...
const contextSources = computed(() => {
  ...
  const charsFor = (...needles: string[]) => sections
    .filter(section => needles.some(needle => `${section.key} ${section.label}`.toLowerCase().includes(needle.toLowerCase())))
    .reduce((sum, section) => sum + section.characters, 0)
  const withSize = (note: string, chars: number) => chars > 0 ? `${note} · ${chars} 字符 / ≈${Math.ceil(chars / 4)} tokens` : note
  return [
    { label: 'Character Card', count: trace.characterCardRuntime ? 1 : 0, note: withSize(trace.characterCardRuntime?.sourceLabel || '未记录', charsFor('角色', 'character')) },
    { label: 'Persona', count: trace.personaName ? 1 : 0, note: withSize(trace.personaName || '未绑定', charsFor('persona', '用户身份')) },
    { label: 'Memory', count: trace.memoryHits.length, note: withSize(trace.memoryHits.length ? '按检索分数注入' : '本轮未命中', charsFor('记忆', '剧情摘要')) },
    { label: 'WorldBook', count: trace.activatedLorebook.length, note: withSize(trace.lorebookEngine ? `${trace.lorebookEngine.evaluatedEntries} 条参与评估` : '旧记录', charsFor('世界书')) },
    { label: 'Recent Messages', count: trace.recentMessages.length, note: withSize('冻结后的最近聊天上下文', charsFor('最近聊天上下文', 'recentMessages')) },
    { label: 'Regex', count: trace.regexPipeline?.activeScripts || 0, note: trace.regexPipeline ? '按 storage / prompt / display 分阶段' : '未记录' },
    { label: 'Agent / Interaction', count: trace.protocolEnabled ? 1 : 0, note: withSize(trace.actionSummary || '无动作摘要', charsFor('互动协议', '动作协议')) }
  ]
})
