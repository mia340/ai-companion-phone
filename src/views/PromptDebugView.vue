...
const contextSources = computed(() => {
  ...
  const statsFor = (...needles: string[]) => {
    const matched = sections.filter(section => needles.some(needle => `${section.key} ${section.label}`.toLowerCase().includes(needle.toLowerCase())))
    return {
      characters: matched.reduce((sum, section) => sum + section.characters, 0),
      tokens: matched.reduce((sum, section) => sum + (section.estimatedTokens ?? Math.ceil(section.characters / 4)), 0)
    }
  }
  const withSize = (note: string, stats: { characters: number; tokens: number }) => stats.characters > 0
    ? `${note} · ${stats.characters} 字符 / ≈${stats.tokens} tokens`
    : note
  return [
    { label: 'Character Card', count: trace.characterCardRuntime ? 1 : 0, note: withSize(trace.characterCardRuntime?.sourceLabel || '未记录', statsFor('角色', 'character')) },
    { label: 'Persona', count: trace.personaName ? 1 : 0, note: withSize(trace.personaName || '未绑定', statsFor('persona', '用户身份')) },
    { label: 'Prompt Preset', count: sections.some(section => `${section.key} ${section.label}`.includes('预设')) ? 1 : 0, note: withSize('按作者顺序进入 System Prompt', statsFor('预设')) },
    { label: 'Memory', count: trace.memoryHits.length, note: withSize(trace.memoryHits.length ? '按检索分数注入' : '本轮未命中', statsFor('记忆', '剧情摘要')) },
    { label: 'WorldBook', count: trace.activatedLorebook.length, note: withSize(trace.lorebookEngine ? `${trace.lorebookEngine.evaluatedEntries} 条参与评估` : '旧记录', statsFor('世界书')) },
    { label: 'Recent Messages', count: trace.recentMessages.length, note: withSize('冻结后的最近聊天上下文', statsFor('最近聊天上下文', 'recentMessages')) },
    { label: 'Regex', count: trace.regexPipeline?.activeScripts || 0, note: trace.regexPipeline ? '按 storage / prompt / display 分阶段' : '未记录' },
    { label: 'Agent / Interaction', count: trace.protocolEnabled ? 1 : 0, note: withSize(trace.actionSummary || '无动作摘要', statsFor('互动协议', '动作协议')) }
  ]
})
const promptTokenEstimate = computed(() => (selected.value?.promptSections || [])
  .reduce((sum, section) => sum + (section.estimatedTokens ?? Math.ceil(section.characters / 4)), 0))
      ...
      <template v-if="selected">
        ...
        <details open class="debug-section">
          ...
          <article v-for="section in selected.promptSections || []" :key="section.key" class="budget-row">
            <div><b>{{ section.label }}</b><small>{{ section.characters }} 字符 · ≈{{ section.estimatedTokens ?? Math.ceil(section.characters / 4) }} tokens{{ section.budget ? ` / 建议 ${section.budget}` : '' }}</small></div>
        ...
        <details open class="debug-section">
          ...
          <p><b>估算上下文 Token：</b>≈{{ promptTokenEstimate }}<template v-if="selected.tokenUsage?.promptTokens"> · API 实际输入 {{ selected.tokenUsage.promptTokens }}</template></p>
          ...
          <div v-if="selected.lorebookEngine?.decisions?.length" class="decision-list">
            ...
            <article v-for="decision in selected.lorebookEngine.decisions" :key="decision.id" :class="['decision-row', `status-${decision.status}`]">
              <div>
                ...
                <small class="decision-meta">
                  <template v-if="decision.activationKind">{{ decision.activationKind }} · </template>
                  <template v-if="decision.matchScore !== undefined">match {{ decision.matchScore }} · </template>
                  <template v-if="decision.recursionDepth">递归 {{ decision.recursionDepth }} · </template>
                  <template v-if="decision.estimatedTokens !== undefined">≈{{ decision.estimatedTokens }} tokens · </template>
                  priority {{ decision.priority ?? 0 }}
                  <template v-if="decision.insertionOrder !== undefined"> · order {{ decision.insertionOrder }}</template>
                  <template v-if="decision.position !== undefined"> · pos {{ decision.position }}</template>
                  <template v-if="decision.probability !== undefined"> · p {{ decision.probability }}%</template>
                  <template v-if="decision.scanDepth !== undefined"> · scan {{ decision.scanDepth }}</template>
                  <template v-if="decision.selectiveLogic"> · {{ decision.selectiveLogic }}</template>
                  <template v-if="decision.useRegex"> · regex</template>
                  <template v-if="decision.caseSensitive"> · case</template>
                  <template v-if="decision.matchWholeWords"> · whole-word</template>
                </small>
                <small v-if="decision.primaryMatches?.length">主关键词：{{ decision.primaryMatches.join('、') }}</small>
                <small v-if="decision.secondaryMatches?.length">辅助关键词：{{ decision.secondaryMatches.join('、') }}</small>
              </div>
