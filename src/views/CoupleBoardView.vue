...
const pendingEventCard = computed(() => game.value?.pendingEvent ? getCoupleBoardEventCard(game.value.pendingEvent.eventCardId) : undefined)
...
async function rollDice() {
  if (!game.value || game.value.pending || game.value.pendingEvent || game.value.status !== 'playing' || diceRolling.value) return
  ...
  const spin = () => {
    ...
    ticks += 1
    if (ticks < 8) {
      diceTimer = window.setTimeout(spin, 58)
      return
    }
    ...
    void (async () => {
      try {
        ...
        showNotice(error instanceof Error ? error.message : '保存棋局失败，请再试一次。')
      } finally {
...
async function acceptEventCard() {
  ...
  const actorGain = pendingEventCard.value.heartDeltaActor
  const otherGain = pendingEventCard.value.heartDeltaOther
  await persist(resolveCoupleBoardEvent(game.value))
  showNotice(`事件卡生效 · 心动 +${actorGain + otherGain}`)
...
async function saveCustomPrompt() {
  try {
    const modes: CoupleBoardMode[] = customDraft.mode === 'both' ? ['chat', 'reality'] : [customDraft.mode]
    const prompt = createCoupleBoardCustomPrompt({
      type: customDraft.type,
      intensity: customDraft.intensity,
      modes,
      text: customDraft.text
    })
    customPrompts.value = [...customPrompts.value, prompt]
    const saved = await saveCoupleBoardPreferences(worldId.value, customPrompts.value)
    customPrompts.value = saved.customPrompts
    customDraft.text = ''
    showNotice('已加入你们的自定义题库。')
  } catch (error) {
    showNotice(error instanceof Error ? error.message : '保存失败。')
  }
}

async function removeCustomPrompt(promptId: string) {
  customPrompts.value = customPrompts.value.filter(prompt => prompt.id !== promptId)
  const saved = await saveCoupleBoardPreferences(worldId.value, customPrompts.value)
  customPrompts.value = saved.customPrompts
  showNotice('已从自定义题库移除。')
}
...
function lastEventLabel() {
  ...
  if (log.eventCardId) return `${actor} 抽到情侣事件卡「${getCoupleBoardEventCard(log.eventCardId)?.title || '心跳事件'}」。`
...
onMounted(async () => {
  ...
  try {
    ...
    const [characterRows, stored, preferences] = await Promise.all([
  ...
  <PhoneFrame>
    <main class="love-game-shell" :class="{ 'is-game': screen === 'game' }">
      ...
      <header class="topbar">
        ...
        <div class="brand-lockup"><small>COUPLE BOARD · V1.1</small><b>心跳飞行棋</b></div>
      ...
      <template v-else-if="screen === 'setup'">
        ...
        <section class="setup-section">
          ...
          <div v-if="characters.length" class="character-row">
            <button v-for="character in characters" :key="character.id" class="character-chip" :class="{ selected: selectedCharacterId === character.id }" @click="selectedCharacterId = character.id">
        ...
        <section class="setup-section deck-section">
          <header><span>04</span><div><b>你们的题库</b><small>自定义题和内置题一起混抽；只保存在本机备份里</small></div></header>
          <button class="deck-card" @click="customEditorOpen = true">
            <span class="deck-icon">✎</span><span><b>{{ customPrompts.length ? `${customPrompts.length} 道自定义题` : '写第一道专属题' }}</b><small>当前设置可抽到 {{ customAvailableCount }} 道 · L4 自定义题同样受 18+ 门禁</small></span><i>›</i>
          </button>
        </section>
      ...
      <template v-else-if="game">
        ...
        <section v-if="game.status === 'playing'" class="dice-dock"><div><small>{{ game.pending ? 'CHALLENGE ACTIVE' : game.pendingEvent ? 'EVENT ACTIVE' : 'ROLL THE DICE' }}</small><b>{{ game.pending || game.pendingEvent ? `${pendingActorName} 还有一张卡没处理` : `轮到 ${currentPlayer?.name}` }}</b></div><button class="dice-button" :class="{ rolling: diceRolling }" :disabled="Boolean(game.pending || game.pendingEvent) || diceRolling" @click="rollDice"><span>{{ diceGlyph }}</span><small>{{ diceRolling ? '...' : '掷' }}</small></button></section>
        ...
        <section v-else class="finish-card">
          ...
          <div class="finish-actions"><button @click="rematch">再来一局</button><button class="secondary" @click="abandonGame">回到设置</button></div>
        </section>
        ...
        <transition name="card-pop">
          <div v-if="game.pending && pendingPrompt" class="challenge-backdrop">
            <section class="challenge-sheet">
              ...
              <div v-if="pendingPrompt.source === 'memory-ai'" class="memory-proof"><span>🕰️</span><p><b>来自真实共同回忆</b><small>AI 只拿已存的 {{ pendingPrompt.memoryEvidenceIds?.length || 0 }} 条记忆作为证据；这道题不会写回或修改原记忆。</small></p></div>
              <div class="consent-note"><span>♡</span><p><b>舒服比输赢重要。</b><small>任何题目都能跳过或换题；现实动作必须由参与者自己同意。</small></p></div>
              <button class="memory-generate" :disabled="memoryGenerating" @click="generateMemoryChallenge"><span>✦</span><p><b>{{ memoryGenerating ? '正在翻找共同回忆…' : '用真实共同回忆重新出题' }}</b><small>会调用你已配置的模型 · 不自动写入记忆</small></p><i>›</i></button>
        ...
        <transition name="card-pop">
          <div v-if="game.pendingEvent && pendingEventCard" class="challenge-backdrop event-backdrop">
            <section class="event-sheet">
              ...
              <div class="event-reward"><span>{{ pendingActorName }} <b>+{{ pendingEventCard.heartDeltaActor }} ♥</b></span><span>{{ game.players[game.pendingEvent.actor === 'user' ? 'partner' : 'user'].name }} <b>+{{ pendingEventCard.heartDeltaOther }} ♥</b></span></div>
      ...
      <transition name="card-pop">
        <div v-if="customEditorOpen" class="challenge-backdrop deck-backdrop" @click.self="customEditorOpen = false">
          <section class="deck-sheet">
            <div class="sheet-handle"></div><header><div><small>YOUR PRIVATE DECK</small><h2>写进你们自己的题库</h2></div><button @click="customEditorOpen = false">×</button></header>
            <div class="deck-form-row"><button :class="{ selected: customDraft.type === 'truth' }" @click="customDraft.type = 'truth'">真心话</button><button :class="{ selected: customDraft.type === 'dare' }" @click="customDraft.type = 'dare'">大冒险</button></div>
            <div class="mini-options"><label>强度<select v-model.number="customDraft.intensity"><option :value="1">L1 纯爱</option><option :value="2">L2 暧昧</option><option :value="3">L3 亲密</option><option :value="4">L4 成人</option></select></label><label>模式<select v-model="customDraft.mode"><option value="both">聊天 + 面对面</option><option value="chat">仅聊天</option><option value="reality">仅面对面</option></select></label></div>
            <textarea v-model="customDraft.text" maxlength="160" placeholder="例如：说一个只有我们两个人懂的小梗，然后告诉对方为什么你一直记得。"></textarea>
            <button class="save-prompt" :disabled="!customDraft.text.trim()" @click="saveCustomPrompt">＋ 加入题库</button>
            <div class="deck-list"><article v-for="prompt in customPrompts" :key="prompt.id"><span :class="prompt.type">{{ prompt.type === 'truth' ? '真心话' : '大冒险' }}</span><p><b>L{{ prompt.intensity }} · {{ prompt.modes.length === 2 ? '双模式' : prompt.modes[0] === 'chat' ? '聊天' : '面对面' }}</b><small>{{ prompt.text }}</small></p><button aria-label="删除" @click="removeCustomPrompt(prompt.id)">×</button></article><div v-if="!customPrompts.length" class="deck-empty">还没有专属题。写一两道只有你们懂的，会比单纯增加题量更有意思。</div></div>
          </section>
        </div>
      </transition>
...
.deck-backdrop{position:absolute;align-items:flex-end;z-index:70}.deck-sheet{width:100%;max-height:86%;overflow:auto;padding:8px 14px 18px;border-radius:27px 27px 20px 20px;background:#fffaf9;color:var(--ink);box-shadow:0 -22px 65px rgba(72,30,47,.24)}.deck-sheet>header{display:flex;justify-content:space-between;align-items:flex-start}.deck-sheet>header div{display:grid}.deck-sheet>header small{font-size:5.5px;letter-spacing:.18em;color:#b2768d}.deck-sheet>header h2{margin:3px 0 0;font:500 18px Georgia,"Songti SC",serif}.deck-sheet>header button{width:28px;height:28px;border:0;border-radius:50%;background:#f3e9ed;color:#805064}.deck-form-row{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:12px}.deck-form-row button{border:1px solid rgba(118,63,83,.08);border-radius:11px;background:#f8eff2;color:#8c5b6d;padding:8px;font-size:7.5px}.deck-form-row button.selected{background:#8b385b;color:#fff}.mini-options{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:6px}.mini-options label{display:grid;gap:3px;padding:7px;border-radius:11px;background:#f8f1f3;font-size:6px;color:#9d7c88}.mini-options select{border:0;background:transparent;color:#66404f;font-size:7.5px;outline:none}.deck-sheet textarea{width:100%;min-height:78px;resize:none;box-sizing:border-box;margin-top:7px;border:1px solid #ead7de;border-radius:13px;background:#fff;padding:10px;color:#5e3948;font:8px/1.6 inherit;outline:none}.deck-sheet textarea:focus{border-color:#c98ca3}.save-prompt{width:100%;margin-top:6px;border:0;border-radius:12px;background:linear-gradient(135deg,#7d3152,#ac496d);color:#fff;padding:9px;font-size:8px;font-weight:800}.save-prompt:disabled{opacity:.4}.deck-list{display:grid;gap:5px;margin-top:11px}.deck-list article{display:grid;grid-template-columns:auto 1fr 24px;align-items:start;gap:7px;padding:8px;border:1px solid rgba(118,63,83,.07);border-radius:12px;background:#fff}.deck-list article>span{padding:4px 5px;border-radius:7px;font-size:5.5px;font-weight:800}.deck-list article>span.truth{background:#eee7f6;color:#756184}.deck-list article>span.dare{background:#f8e1e9;color:#9d4364}.deck-list p{display:grid;margin:0}.deck-list b{font-size:6px;color:#ae8192}.deck-list small{font-size:7px;line-height:1.45;color:#72515e;margin-top:2px}.deck-list article>button{width:22px;height:22px;border:0;border-radius:50%;background:#f5ecef;color:#9d6e7e}.deck-empty{padding:16px 10px;border:1px dashed #dec4ce;border-radius:13px;color:#9d7d89;font-size:7px;line-height:1.55;text-align:center}
