...
async function savePrompt() {
  try {
    const draft = { type: promptDraft.type, intensity: promptDraft.intensity, modes: ['reality' as const], text: promptDraft.text }
  ...
  <PhoneFrame>
    <main class="library-shell">
      <header class="topbar">
        ...
        <div><small>COUPLE BOARD · V2.1 ALPHA</small><b>情侣内容中心</b></div>
      ...
      <template v-else-if="tab === 'prompts'">
        <section class="editor-card">
          ...
          <div class="field-grid single-mode">
            ...
            <div class="mode-lock"><small>游戏模式</small><b>面对面固定</b></div>
          </div>
        ...
        <section class="card-list" v-if="scope === 'mine'">
          <article v-for="prompt in visibleCustomPrompts" :key="prompt.id" class="content-card">
            ...
            <div><small>L{{ prompt.intensity }} · 面对面</small><p>{{ prompt.text }}</p></div>
        ...
        <section class="card-list" v-else>
          <article v-for="prompt in visibleSystemPrompts" :key="prompt.id" class="content-card system" :class="{ disabled: preferences.disabledBuiltinPromptIds.includes(prompt.id) }">
            <span class="kind" :class="prompt.type">{{ prompt.type === 'truth' ? '真心话' : '大冒险' }}</span>
            <div><small>L{{ prompt.intensity }} · {{ coupleBoardPromptThemeLabel(prompt) }} · 面对面</small><p>{{ prompt.text }}</p></div>
      ...
      <template v-else>
        ...
        <section class="history-list">
          <article v-for="row in archive.entries" :key="row.id" class="history-card" :class="{ expanded: expandedArchiveId === row.id }">
            <button class="history-main" @click="expandedArchiveId = expandedArchiveId === row.id ? '' : row.id"><span class="history-heart">♥</span><div><small>{{ formatDate(row.finishedAt) }} · {{ row.mode === 'chat' ? '旧版聊天局' : '面对面' }} · L{{ row.intensity }}</small><b>和 {{ row.characterName }} 的一局</b><p>{{ row.totalHearts }} 点心动 · {{ row.completed }} 个完成 · {{ row.eventCount }} 张事件卡</p></div><i>{{ expandedArchiveId === row.id ? '⌃' : '⌄' }}</i></button>
