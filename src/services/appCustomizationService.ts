...
export const HOME_LAYOUT_REVISION = 14
...
export interface HomeLayoutDiagnosticItem {
  id: string
  type: HomeLayoutItem['type']
  key: string
  label: string
  x: number
  y: number
...
export interface HomeLayoutDiagnosticPage {
  pageIndex: number
  itemCount: number
  occupiedCells: number
  items: HomeLayoutDiagnosticItem[]
  issues: string[]
}

export interface HomeLayoutDiagnostics {
  revision: number
  pageCount: number
  dockAppKeys: HomeAppKey[]
  issues: string[]
  pages: HomeLayoutDiagnosticPage[]
}
...
const APP_CATALOG: readonly HomeAppDefinition[] = [
  ...
  { key: 'wardrobe', label: '穿搭', route: '/app/穿搭', icon: 'wardrobe', tone: ['#b6748d', '#e6b7c8'] },
...
function normalizeLayoutPages(
  ...
  if (!pages.length) {
    // Grid V5+ 的 homeLayoutPages 是 Launcher 稳定事实源。只要该字段已存在，
    // 即使清洗后为空，也不能再用兼容字段 homeAppKeys/homeWidgetKeys 把幽灵项目复活。
    if (canonicalLayout) return [{ items: [] }]
    return createLegacyLayout(legacyPages, homeWidgetKeys)
  }

  // 仅旧数据迁移时，兼容字段还承担成员集合职责，需要把缺失项目补入空位。
  // 一旦存在 canonical homeLayoutPages，页面内容本身就是事实源；兼容字段只能由布局反推。
  if (!canonicalLayout) {
...
export async function loadHomeAppearance(worldId: string): Promise<HomeAppearancePreferences> {
  ...
  // Launcher revision 14 自愈：历史版本可能残留 Dock/桌面重复项、重复 Widget、空页或缺少新 App。
  ...
  if (row) {
    ...
    const pagesChanged = JSON.stringify(storedPages) !== JSON.stringify(repaired.homeLayoutPages)
    if (pagesChanged || storedRevision !== HOME_LAYOUT_REVISION) {
      await saveHomeAppearance(worldId, repaired)
    }
  }
...
function itemDiagnosticLabel(item: HomeLayoutItem) {
  if (item.type === 'folder') return item.name || '文件夹'
  if (item.type === 'widget') return WIDGET_CATALOG.find(row => row.key === item.key)?.label || item.key
  return getHomeAppDefinition(item.key)?.label || item.key
}

/**
 * 纯数据层 HomeLayout Inspector。它不依赖 DOM，可用于测试、备份预检和 UI 诊断。
 * DOM 是否真正绘制由 HomeScreen 在这个结果上再补一层运行时信息。
 */
export function inspectHomeLayout(value: HomeAppearancePreferences): HomeLayoutDiagnostics {
  const normalized = normalizeHomeAppearance(value)
  const dockSet = new Set(normalized.dockAppKeys)
  const seenApps = new Map<HomeAppKey, string>()
  const seenWidgets = new Map<HomeWidgetKey, string>()
  const issues: string[] = []
  const pages = normalized.homeLayoutPages.map<HomeLayoutDiagnosticPage>((page, pageIndex) => {
    const pageIssues: string[] = []
    const items = page.items.map<HomeLayoutDiagnosticItem>(item => {
      const itemIssues: string[] = []
      const inBounds = item.x >= 0 && item.y >= 0 && item.w >= 1 && item.h >= 1
        && item.x + item.w <= HOME_GRID_COLUMNS && item.y + item.h <= HOME_GRID_ROWS
      if (!inBounds) itemIssues.push('越界')
      const overlaps = page.items.filter(other => other.id !== item.id && cellsOverlap(item, other))
      if (overlaps.length) itemIssues.push(`与 ${overlaps.map(other => other.id).join('、')} 重叠`)

      const appKeys = item.type === 'app' ? [item.key] : item.type === 'folder' ? item.appKeys : []
      for (const key of appKeys) {
        if (dockSet.has(key)) itemIssues.push(`${key} 同时存在于 Dock`)
        const seenAt = seenApps.get(key)
        if (seenAt) itemIssues.push(`${key} 重复出现（首次 ${seenAt}）`)
        else seenApps.set(key, `第 ${pageIndex + 1} 页/${item.id}`)
      }
      if (item.type === 'widget') {
        const seenAt = seenWidgets.get(item.key)
        if (seenAt) itemIssues.push(`${item.key} Widget 重复（首次 ${seenAt}）`)
        else seenWidgets.set(item.key, `第 ${pageIndex + 1} 页/${item.id}`)
      }
      if (item.type === 'folder' && item.appKeys.length < 2) itemIssues.push('文件夹成员少于 2 个')
      return {
        id: item.id,
        type: item.type,
        key: item.type === 'folder' ? item.appKeys.join(',') : item.key,
        label: itemDiagnosticLabel(item),
        x: item.x,
        y: item.y,
        w: item.w,
        h: item.h,
    ...
    for (const item of items) pageIssues.push(...item.issues.map(issue => `${item.label}：${issue}`))
    if (!items.length) pageIssues.push('空页面')
    return {
      pageIndex,
      itemCount: items.length,
      occupiedCells: page.items.reduce((sum, item) => sum + item.w * item.h, 0),
      items,
      issues: pageIssues
    }
  })
  for (const page of pages) issues.push(...page.issues.map(issue => `第 ${page.pageIndex + 1} 页：${issue}`))
  return { revision: normalized.homeLayoutRevision, pageCount: pages.length, dockAppKeys: [...normalized.dockAppKeys], issues, pages }
}

/**
 * 结构修复只处理可以确定的脏数据：Dock/桌面重复、重复 App/Widget、空页。
 * 不会为了“塞满第一页”打乱用户主动保留的分页。
 */
export function repairHomeLayoutIntegrity(value: HomeAppearancePreferences): HomeAppearancePreferences {
  const normalized = normalizeHomeAppearance(value)
  const dockSet = new Set(normalized.dockAppKeys)
  const seenApps = new Set<HomeAppKey>()
  const seenWidgets = new Set<HomeWidgetKey>()
  ...
  for (const page of normalized.homeLayoutPages) {
    const items: HomeLayoutItem[] = []
    for (const source of sortItemsByVisualOrder(page.items)) {
      if (source.type === 'widget') {
        if (seenWidgets.has(source.key)) continue
        const fit = canPlace(items, source.x, source.y, source.w, source.h)
          ? { x: source.x, y: source.y }
          : findFirstFit(items, source.w, source.h)
        if (!fit) continue
        items.push({ ...source, ...fit })
        seenWidgets.add(source.key)
        continue
      }

      if (source.type === 'app') {
        if (dockSet.has(source.key) || seenApps.has(source.key)) continue
        const fit = canPlace(items, source.x, source.y, 1, 1)
          ? { x: source.x, y: source.y }
          : findFirstFit(items, 1, 1)
        if (!fit) continue
        items.push({ ...source, ...fit })
        seenApps.add(source.key)
        continue
      }

      const members = source.appKeys.filter(key => !dockSet.has(key) && !seenApps.has(key))
      if (!members.length) continue
      const fit = canPlace(items, source.x, source.y, 1, 1)
        ? { x: source.x, y: source.y }
        : findFirstFit(items, 1, 1)
      if (!fit) continue
      if (members.length === 1) items.push(createAppItem(members[0], fit.x, fit.y))
      else items.push({ ...source, appKeys: members, ...fit })
      members.forEach(key => seenApps.add(key))
    }
  ...
  return normalizeHomeAppearance({
    ...
    homeLayoutPages: compacted,
    homeAppKeys: collectPageAppKeys(compacted),
    homeWidgetKeys: compacted.flatMap(page => page.items
      .filter((item): item is HomeLayoutWidgetItem => item.type === 'widget')
      .map(item => item.key)),
    homePageKeys: deriveLegacyPageKeys(compacted)
  })
...
export function normalizeHomeAppearance(value: HomeAppearanceInput): HomeAppearancePreferences {
  ...
  const dockAppKeys = normalizeAppKeys(value.dockAppKeys, fallback.dockAppKeys).slice(0, 4)
  const dockSet = new Set(dockAppKeys)
  const homeWidgetKeys = normalizeWidgetKeys(value.homeWidgetKeys, fallback.homeWidgetKeys)
  // 与真实手机一致：同一个 App 进入 Dock 后不再保留一份桌面副本。
  // 这也会清掉旧版本可能遗留在幽灵页里的 Dock 重复项。
  const requestedHomeAppKeys = normalizeAppKeys(value.homeAppKeys, fallback.homeAppKeys).filter(key => !dockSet.has(key))
  const legacyPages = normalizeHomePageKeys(value.homePageKeys, requestedHomeAppKeys, homeWidgetKeys.length > 0)
  const rawLayoutPages = normalizeLayoutPages(
    value.homeLayoutPages,
  ...
  if (storedRevision < 14) migratedPages = migrateWardrobeLauncherApp(migratedPages)
...
function migrateWardrobeLauncherApp(pages: HomeLayoutPage[]): HomeLayoutPage[] {
  const next = pages.map(page => ({ items: page.items.map(item => ({ ...item })) as HomeLayoutItem[] }))
  if (collectPageAppKeys(next).includes('wardrobe')) return next
  const item = createAppItem('wardrobe', 0, 0)
  placeOnAnyPage(next, { ...item, x: undefined, y: undefined }, 0)
  return next
}
