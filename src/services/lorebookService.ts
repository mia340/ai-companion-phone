...
function makeCandidate(entry: LorebookEntry, options: {
  ...
  return {
    ...
    primaryMatches: options.primaryMatches?.slice(),
    secondaryMatches: options.secondaryMatches?.slice()
  }
}
  ...
  for (const item of entries.filter(entry => entry.enabled)) {
    ...
    if (details.matched && probabilityPasses(item)) {
      addCandidate(makeCandidate(item, {
        ...
        kind: 'keyword',
        score: details.score,
        primaryMatches: details.primaryKeys,
        secondaryMatches: details.secondaryKeys
      }))
    }
  }
  ...
  const engineDecisions: LorebookEngineDebugDecision[] = entries
    ...
    .map(item => {
      ...
      const common = {
        ...
        primaryMatches: candidate?.primaryMatches?.slice(),
        secondaryMatches: candidate?.secondaryMatches?.slice(),
        selectiveLogic: item.selective ? normalizedSelectiveLogic(item.selectiveLogic) : undefined,
        scanDepth: item.scanDepth ?? (item.lorebookId ? books.get(item.lorebookId)?.scanDepth : undefined) ?? 16,
