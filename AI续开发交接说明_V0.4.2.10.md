...
- V0.4.2.9 已修复 `CharacterCreate.vue` 中直接展示 `{{user}}` 时的 Vue 模板解析问题。
- Windows 实际构建继续发现 `UserProfileView.vue:332` 的动态表达式内仍直接包含 `{{user}}`，Vite 报 `Unterminated string constant`。
- V0.4.2.10 将该兜底文案移到 `<script setup>` 常量 `characterCardUserTemplateFallback`，模板只引用变量。
- 三个相关页面已重新复核：静态字面量使用 `v-pre`；动态表达式不得直接嵌入 `{{user}}`。
- 角色卡自带 `{{user}}` → 角色专属 Persona → 自动绑定聊天功能保留。
- 角色专属 Persona 不得污染全局 Persona，也不能把角色卡作者写的用户模板自动升级为现实用户事实。
- IndexedDB 仍为 V8，备份格式仍为 V7。


## V0.4.2.8 构建修复补充

- V0.4.2.7 创建角色时新增了 `characters / conversations / messages / lorebookEntries / personas / chatSettings` 六表原子事务。
- Dexie 4 的 TypeScript 重载对逐表参数数量有限制，V0.4.2.7 因此在 `CharacterCreate.vue` 报 `TS2554: Expected 3-7 arguments, but got 8`。
- V0.4.2.8 改为 `db.transaction('rw', [六张表], async () => ...)`，功能不变。
- IndexedDB 仍为 V8，备份格式仍为 V7。
...
```text
请先阅读《AI续开发交接说明_V0.4.2.8.md》，再检查我上传的最新项目 ZIP。
...
## V0.4.2.6 新增：Persona 资源导入导出与 Tavo 兼容
...
## V0.4.2.6 创建期 JSON 存储兼容修复

用户实机导入 Tavo Character Card V2（例如“谢无矣”）时，JSON 能正确识别并填入表单，但点击创建会出现：

```text
DataCloneError: Failed to execute 'add' on 'IDBObjectStore': [object Array] could not be cloned.
```

根因不是角色卡损坏，而是导入结果进入 Vue 深响应式 `ref` 后，`alternateGreetings`、`tags`、`exampleDialogues`、内嵌世界书 keywords 等数组可能变成 Proxy；IndexedDB structured clone 不能保存 Proxy。

本版要求：

- 角色卡导入预览使用 `shallowRef`，避免深度代理社区 JSON。
- 角色、角色专属世界书写库前必须经过 `toPlainStorageValue()`。
- Persona 的 `extraFields`、tags 等导入数据也必须在写库前去 Proxy。
- Tavo 常见 `alternate_greetings: null`、`character_book: null`、空 tags 继续规范化为空数组。
- 角色卡 description 中若内置 `{{user}}` 模板，该模板不是当前用户事实；只有当前 Persona、真实聊天历史或可信长期记忆确认的内容才能当作用户事实。
- 不要重新引入 `src/services/characterCardImportService.js` 这类编译产物；源码目录只保留 TypeScript，避免扩展名解析歧义。

数据库仍是 V8，备份格式仍是 V7。


## V0.4.2.7 新增：角色卡 {{user}} 与角色专属 Persona

### 产品规则

角色卡中的 `{{user}}` 是作者为当前角色/剧情预设的用户身份。它不等于用户的全局现实资料。

当前实现：
- 导入角色卡时解析 `description` 中独立的 `{{user}}:` 缩进段，也支持 `{{user}}: 内容` 同行写法。
- 创建角色页直接展示该模板。
- 默认勾选“使用卡内 {{user}}”，并在创建角色的同一事务中创建角色专属 Persona。
- 新私聊的 `chatSettings.personaId` 自动绑定该 Persona。
- `UserPersona.personaScope = 'character'`，并保存 `boundCharacterId / boundCharacterName / sourceUserTemplate / isCardTemplate`。
- `Character` 保存 `embeddedUserTemplate / embeddedUserPersonaId`。
- 角色专属 Persona 不能成为全局默认 Persona。
- 聊天 Persona 列表只显示全局 Persona 与当前角色自己的专属 Persona。
- 角色卡编辑页可以查看原始 `{{user}}` 并创建/更新 Persona。
- “我的资料”页显示全局 Persona 与角色卡专属 Persona 概览。
- 角色卡导出 V2 JSON 时会重新带上 `{{user}}` 模板，减少往返导入信息丢失。

### 事实边界

只有当前聊天实际选择了该角色专属 Persona 时，卡内 `{{user}}` 才能作为当前剧情中的用户事实。
切换为其他 Persona 后，不得继续使用卡内职业、习惯、经历、关系等事实。
卡内 Persona 也不得写入用户全局资料或其他角色的 Persona。

### 针对 Tavo 样本的验证

使用用户提供的 `Tavo_谢无矣_20251027T1128(1).json` 验证：
- 识别角色：谢无矣
- 自动检测 `{{user}}`
- 生成 Persona 名：谢无矣 · 原卡用户
- 结构化识别职业：娱乐圈新人女演员
- 结构化识别外貌：身娇体软 / 肤白貌美
- 结构化识别经历并保留完整原始模板
- `structuredClone` 可通过
- `对{{user}}:` 这类角色侧描述不会被误识别成用户模板

### 数据兼容

- IndexedDB 仍为 V8。
- 备份格式仍为 V7。
- 没有新增表或索引，只给 Character / UserPersona 增加可选字段。
- 旧数据无需迁移。

### 后续建议

下一阶段优先继续验证：
1. 更多 Tavo 卡 `{{user}}` 格式的识别率。
2. “使用我的 Persona / 使用原卡 {{user}} / 合并两者”三模式。
3. 卡内 `{{user}}` 中复杂 YAML/自定义字段的结构化映射。
4. 角色卡 PNG 元数据读取。
5. Character Book 更完整的 position/depth/probability 等兼容。
